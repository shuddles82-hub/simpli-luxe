...
// the site.
// `fallbackFilterFormula` lets a caller ask for a formula that
// references a field which may not exist yet on every base (e.g. a
// "Publish Date" scheduling field) without risking the whole series
// going empty: if Airtable rejects the primary formula (unknown
// field), this retries once with the safer fallback before giving up.
async function fetchRecords(table, { sortField, filterFormula, fallbackFilterFormula } = {}) {
  ...
  function buildParams(formula) {
    ...
    params.set('filterByFormula', formula || "TRIM({Status})='Published'");
  ...
  for (const name of names) {
    const url = `https://api.airtable.com/v0/${baseId}/${encodeURIComponent(name)}?${buildParams(filterFormula).toString()}`;
    try {
      ...
      if (fallbackFilterFormula) {
        const fallbackUrl = `https://api.airtable.com/v0/${baseId}/${encodeURIComponent(name)}?${buildParams(fallbackFilterFormula).toString()}`;
      ...
      // table not found under this name (or fallback also failed); try the next
    } catch {
...
// Insider Weekly: the Luxe Insider content hub, five recurring pillars
// (Daily Affirmation, Sunday Reset, Money Monday, Wellness Wednesday,
// Weekend Luxury Guide). Field names mirror the "Content Queue" AI
// pipeline's Draft* fields, so approved queue items can be copied over
// with almost no re-typing.
// A record with a future "Publish Date" stays hidden until that date
// and time passes, even if Status is already Published — lets Staci
// approve Money Monday / Wellness Wednesday content in advance and
// have it appear automatically on the right day. An empty or absent
// Publish Date publishes immediately, so existing records are unaffected.
export async function getInsiderWeekly() {
  const records = await fetchRecords('Insider Weekly', {
    filterFormula:
      "AND(TRIM({Status})='Published', OR({Publish Date}='', IS_BEFORE({Publish Date}, NOW())))",
    fallbackFilterFormula: "TRIM({Status})='Published'",
  });
  records.sort((a, b) => new Date(b.createdTime) - new Date(a.createdTime));
  return records.map((r, i) => {
    const f = r.fields || {};
...
export async function getShopCollections({ featuredOnly = false } = {}) {
  const filterFormula = featuredOnly
    ? "AND(TRIM({Status})='Published',{Featured}=1)"
